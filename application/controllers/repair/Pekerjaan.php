...
class Pekerjaan extends CI_Controller {
	...
	public function __construct()
    {
        ...
        $this->load->model(array('mrepairpekerjaan','mkantor','mrepairdetail'));
	...
	{		
		$data = array(
			...
			'table'		=> Mrepairpekerjaan::select('repair_pekerjaan.*',
											'kantor.nama_k')
							->leftjoin('kantor', 'kantor.id_k','=','repair_pekerjaan.id_k')
							->get(),
		);
		return view('repair.pekerjaan.index',$data);
	}
	...
	public function create()
	{
		...
		return view('repair.pekerjaan.form',$data);
	}
	...
	{	
        ...
        else
        {
            ...
            redirect('repair/pekerjaan');
        }
    }

	public function bast($id="")
	{
		$filename = "BASTREPAIR".date('dmY').".doc";
		...
		$data = array(
			'title'		=> 'BAST Repair #'.$id,
			'detail'	=> Mrepairpekerjaan::select('repair_pekerjaan.*',
											'kantor.nama_k')
							->leftjoin('kantor', 'kantor.id_k','=','repair_pekerjaan.id_k')
							->find($id),
			'repair_detail'	=> Mrepairdetail::leftjoin('repair_subkon', 'repair_subkon.id_repair_subkon','=','repair_detail.id_repair_subkon')
							->where('id_repair_pekerjaan',$id)->get(),
		);

		$this->load->view('repair/pekerjaan/bast',$data);
	}
	...
	public function edit($id="")
	{
		$data = array(
			...
			'msg'		=> '',
			'form'		=> $this->form('update',Mrepairpekerjaan::find($id)),
	...
	public function update($id)
	{
		...
		if ($this->form_validation->run() == FALSE)
        {
            $data = array(
				...
				'msg_s'		=> $this->session->flashdata('msg_s'),
				'msg_py'		=> $this->session->flashdata('msg_py'),
				'detail'	=> Mrepairpekerjaan::select('pekerjaan.*',
												'kantor.nama_k')
	...
	{
		if ($type=="insert") {
			$form = $this->form_builder->open_form(array('action' => 'repair/pekerjaan/insert'));
		}elseif ($type=="update") {
			$form = $this->form_builder->open_form(array('action' => 'repair/pekerjaan/update/'.$data->id_repair_pekerjaan));
		}
